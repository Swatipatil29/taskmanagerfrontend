...
export default function List(){
    ...
    useEffect(()=>{
        ...
        useEffect(() => {
            const fetchData = async () => {
                try {
                    const response = await  axios.get("http://localhost:3050/api/getAlltasks")
                    dispatch({ type: 'LIST_TASK', payload: response.data });
                } catch (error) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:3050/api/tasks');
    ...
    const handleRemove = async (id) => {
        ...
        if (confirm) {
            try {
                await axios.delete(`http://localhost:3050/api/tasks/${id}`);
    ...
    const handleSelect = async (id)=>{
        try{
            const response = await axios.get(`http://localhost:3050/api/tasks/${id}`)
    ...
    const handleSave = async ()=>{
        try{
            const response = await axios.put(`http://localhost:3050/api/tasks/${task._id}`, task)
    ...
    return(
        <div className="col-md-10">
            <h2 style={{marginTop: "30px"}}>Listing tasks - {tasks.data.length}</h2>
            ...
            <ul style={{marginTop: "20px"}} className="list-group">
